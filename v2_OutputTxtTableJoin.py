...
##          features, and write a text table file to store the information. Also, the output
##          table can be joined to the input feature
## Acknowledgment: The codes I looked at are by:
##          Miles Hitchen (the greatest help, in VB, found in Forums.Esri.Com & old ArcScripts),
...
##               5) Specify an output file path and name
##               6) Check if one wants the output added to the view & join to the input feature
######################################################################
...
try:
    ...
    outputTXT = arcpy.GetParameterAsText(4)
    joinYesNo = arcpy.GetParameterAsText(5)
##    inputFC = 'C:/Temp/Python/NthNeighbors/ThiessenPolygons.shp'
##    idfieldName = "Id"
##    outputTXT = "OutputTab.csv"
##    newField = "Neighbors"
##    newField2 = "Neighbors2"
    ...
    # Prepare the output text table - comma delimited. Create a header
    writeTXT = open(outputTXT, "w")
    if newField2 <> "":
        writeTXT.write("ID," + newField + "," + newField2 + "\n")
    else:
        writeTXT.write("ID," + newField + "\n")
    
    srcFCCursor = arcpy.SearchCursor(lyrFC)
    srcFCRow = srcFCCursor.next()

    while srcFCRow:
        cursrcFCRowidField = str(getattr(srcFCRow, idfieldName))
        ...
        rowOID = srcFCRow.getValue(inDesc.OIDFieldName)
        ...
        if inDesc1.FIDSet <> "":
            ...
            if newField2 <> "":
                writeTXT.write(cursrcFCRowidField + "," + neighborList + "," + neighborList2 + "\n")
            else:
                writeTXT.write(cursrcFCRowidField + "," + neighborList + "\n")
        srcFCRow = srcFCCursor.next()
    writeTXT.close()
    
    # Add the table to the content and also join to the input feature
    if joinYesNo == "true":
        arcpy.AddJoin_management(inputFC,idfieldName,outputTXT,idfieldName)

    # Clean up cursors and variables to avoid data locks
    del srcFCCursor, srcFCRow, srcCursor, srcRow
    if newField2 <> "":
